...
app.post('/api/generate-idea', async (req, res) => {
  try {
    ...
    const serviceLine = isService && productName
      ? `\nЭто конкретная услуга клиники — идея ДОЛЖНА явно называть именно эту услугу ("${productName}") в озвучке/диалоге персонажа, а не быть общим роликом про клинику/стоматологию вообще.${servicePhoneMatch ? ` Если в идее упоминается номер телефона — это ДОЛЖЕН быть именно "${servicePhoneMatch[1].trim()}", без выдумывания другого номера.` : ''}${serviceClinicMatch ? ` Если в идее упоминается название клиники — это ДОЛЖНО быть именно "${serviceClinicMatch[1].trim()}", без выдумывания другого названия.` : ''} Медицинские термины в названии услуги нельзя переводить или переосмысливать по-своему: "All-on-4" — это протокол "все на четырёх" (несъёмный протез всей челюсти с опорой на 4 импланта), а НЕ "четыре в одном"; аналогично "All-on-6" — "все на шести". В озвучке говори "все на четырёх"/"все на шести" или оставляй оригинальное "All-on-4".`
      : '';
